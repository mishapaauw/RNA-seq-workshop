...
DEGs_list <- DEGs %>% filter(significance == "Significantly upregulated" | significance == "Significantly downregulated") %>% pull(genes)
```
...
```{R filename = "R"}
universe_list <- DEGs %>% filter(baseMean > 1) %>% pull(genes)
head(universe_list)
...
```{R filename = "R"}
gene_list <- DEGs$log2FoldChange # <1>
names(gene_list) <- DEGs$genes # <2>
...
1. By picking `1` here, we will plot the GO term with the lowest p-value. Make sure to explore a few more than just the first one!
